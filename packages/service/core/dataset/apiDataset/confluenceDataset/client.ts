...
class ConfluenceClient {
  ...
  // 获取指定页面的信息
  async getPageById(pageId: string, bodyFormat: string = 'atlas_doc_format'): Promise<Page> {
    try {
  ...
  async downloadAttachmentToBase64(link: string, mediaType: string | undefined): Promise<string> {
    try {
      ...
      const imageType = mediaType || getFileContentTypeFromHeader(response.headers['content-type']);
      // 构造 Base64 URL
...
const getFileContentTypeFromHeader = (header: string): string | undefined => {
