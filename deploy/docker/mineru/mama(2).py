...
try:
    ...
    from mineru.model.mfr.pp_formulanet_plus_m.predict_formula import FormulaRecognizer as PPFormulaRecognizer
    
    _orig_pp_batch_predict = PPFormulaRecognizer.batch_predict
    _pp_sig = inspect.signature(_orig_pp_batch_predict)
    
    @wraps(_orig_pp_batch_predict)
    def _adaptive_pp_batch_predict(self, images_mfd_res: list, images: list, batch_size: int = 64, interline_enable: bool = True) -> list:
        if 'interline_enable' not in _pp_sig.parameters and not interline_enable:
            filtered_mfd_res = []
            for res in images_mfd_res:
                mask = res.boxes.cls != 1
                import copy
                res_copy = copy.copy(res)
                res_copy.boxes = res.boxes[mask]
                filtered_mfd_res.append(res_copy)
            images_mfd_res = filtered_mfd_res
        args_to_pass = [images_mfd_res, images]
        kwargs_to_pass = {'batch_size': batch_size}
        
        if 'interline_enable' in _pp_sig.parameters:
            kwargs_to_pass['interline_enable'] = interline_enable
            
        return _orig_pp_batch_predict(self, *args_to_pass, **kwargs_to_pass)

    PPFormulaRecognizer.batch_predict = _adaptive_pp_batch_predict
    logger.info("Successfully applied adaptive proxy patch to FormulaRecognizer")
except (ImportError, AttributeError, Exception) as e:
    logger.warning(f"Failed to apply adaptive FormulaRecognizer patch: {e}")
# ==============================================================================
...
# Batch processing configuration for large PDFs
# Can be overridden via environment variables or API parameters
BATCH_SIZE = int(os.environ.get("MINERU_BATCH_SIZE", "100"))  # Pages per batch
MAX_CONCURRENT_BATCHES = int(os.environ.get("MINERU_MAX_CONCURRENT_BATCHES", "2"))  # Maximum concurrent batches
AUTO_BATCH_THRESHOLD = int(os.environ.get("MINERU_AUTO_BATCH_THRESHOLD", "50"))  # Auto-enable batching threshold
BATCH_TIMEOUT = 900  # 15 minutes timeout per batch

# Runtime mode configuration - only one mode can be active at a time
RUNTIME_MODE = os.environ.get("MINERU_MODE", "hybrid").lower()  # pipeline, vlm, or hybrid
SERVER_URL = os.environ.get("MINERU_SERVER_URL", None)  # For VLM client mode
VLM_BACKEND = os.environ.get("MINERU_VLM_BACKEND", "hybrid-auto-engine")  # Default backend

# Backend configurations
PIPELINE_BACKENDS = ["pipeline"]
# VLM backends should use the "vlm-" prefix format that aio_do_parse expects
VLM_BACKENDS = ["vlm-vllm-async-engine", "vlm-auto-engine"]
# Hybrid backends
HYBRID_BACKENDS = ["hybrid-auto-engine"]
...
if RUNTIME_MODE == "pipeline":
    ...
    VLM_BACKEND = os.environ.get("MINERU_VLM_BACKEND", "hybrid-auto-engine")
    ACTIVE_BACKENDS = [VLM_BACKEND]
    DEFAULT_BACKEND = VLM_BACKEND
    logger.info(f"Running in HYBRID mode with backend: {DEFAULT_BACKEND}")
elif RUNTIME_MODE == "vlm":
    ACTIVE_BACKENDS = [VLM_BACKEND] if VLM_BACKEND in VLM_BACKENDS else ["vlm-auto-engine"]
    ...
    RUNTIME_MODE = "hybrid"
    VLM_BACKEND = "hybrid-auto-engine"
    ACTIVE_BACKENDS = [VLM_BACKEND]
    DEFAULT_BACKEND = VLM_BACKEND

ALL_BACKENDS = PIPELINE_BACKENDS + VLM_BACKENDS + HYBRID_BACKENDS  # Keep for reference
...
HYBRID_BACKENDS_SET = frozenset(HYBRID_BACKENDS)
...
async def root():
    ...
    return {
        ...
        "mode_info": {
            ...
            "description": "Pipeline OCR-based processing" if RUNTIME_MODE == "pipeline" else f"{RUNTIME_MODE.capitalize()}-based processing using {DEFAULT_BACKEND}",
...
def prepare_backend_params(backend: str, **kwargs) -> Dict[str, Any]:
    ...
    # Backend-specific parameters
    is_vlm = backend in VLM_BACKENDS_SET
    is_hybrid = backend in HYBRID_BACKENDS_SET
    
    if (is_vlm or is_hybrid) and config.get("supports_server_params"):
...
# ============================================================================
# Batch Processing Functions for Large PDFs
# ============================================================================

async def get_pdf_page_count(pdf_bytes: bytes, request_id: str = "unknown") -> int:
    """
    检测PDF总页数
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[{request_id}] PDF has {page_count} pages (using PyPDF2)")
            return page_count
            
        except ImportError:
            logger.warning(f"[{request_id}] No PDF library available for page count detection, assuming single batch")
            return 0  # 返回0表示无法检测，将使用单批次处理
            
    except Exception as e:
        logger.error(f"[{request_id}] Failed to detect PDF page count: {str(e)}")
        return 0  # 返回0表示检测失败，将使用单批次处理


def calculate_smart_batch_params(total_pages: int, request_id: str = "unknown") -> dict:
    """
    根据PDF总页数智能推算最优的批处理参数
    ...
    Returns:
        dict: {
            'batch_size': int,  # 每批处理的页数
            'max_concurrent': int,  # 最大并发批次数  
            'reason': str  # 选择此配置的原因
        }
    """
    if total_pages <= 100:
        ...
        result = {
            ...
            'max_concurrent': 1,
            'reason': f'Small PDF ({total_pages} pages), using single batch'
        }
    elif total_pages <= 200:
        # 100-200页：中等批次，低并发
        # 目标：2个批次，1-2并发
        batch_size = max(80, total_pages // 2)
        result = {
            'batch_size': batch_size,
            'max_concurrent': 2,
            'reason': f'Medium PDF ({total_pages} pages), balanced approach'
        }
    elif total_pages <= 500:
        # 200-500页：标准批次，适中并发
        # 目标：3-5个批次，2-3并发
        batch_size = 100
        max_concurrent = min(3, max(2, total_pages // 200))
        result = {
            'batch_size': batch_size,
            'max_concurrent': max_concurrent,
            'reason': f'Standard PDF ({total_pages} pages), optimal batch size'
        }
    elif total_pages <= 1000:
        # 500-1000页：小批次，高并发
        # 目标：6-10个批次，3-4并发
        batch_size = 80
        max_concurrent = min(4, max(3, total_pages // 300))
        result = {
            'batch_size': batch_size,
            'max_concurrent': max_concurrent,
            'reason': f'Large PDF ({total_pages} pages), smaller batches for better parallelism'
        }
    else:
        # >1000页：超小批次，最高并发
        # 目标：15+个批次，4-5并发
        batch_size = 60
        max_concurrent = min(5, max(4, total_pages // 400))
        result = {
            'batch_size': batch_size,
            'max_concurrent': max_concurrent,
            'reason': f'Very large PDF ({total_pages} pages), maximum parallelism'
        }
    
    logger.info(
        f"[{request_id}] Smart batch params: "
        f"pages={total_pages}, batch_size={result['batch_size']}, "
        f"max_concurrent={result['max_concurrent']}, "
        f"estimated_batches={-(-total_pages // result['batch_size'])}, "  # 向上取整
...
def calculate_batches(total_pages: int, batch_size: int = BATCH_SIZE) -> list:
    """
    将页面范围分成多个批次
    ...
    Returns:
        List[Tuple[int, int]]: 批次列表，每个元素为 (start_page, end_page)
    """
    if total_pages <= 0:
        return []
...
async def process_single_batch(
    ...
    **parse_params
    ...
    Returns:
        dict: 批次处理结果，包含result_dir和其他元数据
    """
    batch_request_id = f"{request_id}_b{batch_id}"
    logger.info(
        f"[{batch_request_id}] Processing batch {batch_id}: "
        f"pages {start_page}-{end_page} ({end_page - start_page + 1} pages)"
    )
    
    try:
        # 创建批次专用的输出目录
        batch_output_dir = os.path.join(output_base_dir, f"batch_{batch_id}")
        os.makedirs(batch_output_dir, exist_ok=True)
        
        # 更新解析参数，添加页面范围
        batch_parse_params = parse_params.copy()
        batch_parse_params['start_page_id'] = start_page
        batch_parse_params['end_page_id'] = end_page
        
        # 调用aio_do_parse处理此批次
        backend = batch_parse_params.pop('backend', 'pipeline')
        parse_method = batch_parse_params.pop('parse_method', 'auto')
        lang = batch_parse_params.pop('lang', 'ch')
        formula_enable = batch_parse_params.pop('formula_enable', True)
        table_enable = batch_parse_params.pop('table_enable', True)
        ...
        # 确定结果目录
        result_dir = get_result_directory(batch_output_dir, file_stem, backend, parse_method)
        
        if not os.path.exists(result_dir):
            raise FileNotFoundError(f"Batch {batch_id} result directory not found: {result_dir}")
        
        logger.info(f"[{batch_request_id}] Batch {batch_id} completed successfully")
        ...
        logger.error(f"[{batch_request_id}] Batch {batch_id} failed: {str(e)}", exc_info=True)
        return {
            "batch_id": batch_id,
            "start_page": start_page,
            "end_page": end_page,
...
async def process_batches_parallel(
    ...
    **parse_params
    ...
    Args:
        batches: 批次列表 [(start_page, end_page), ...]
        pdf_bytes: PDF字节数据
        file_stem: 文件名（不含扩展名）
        output_base_dir: 输出基础目录
        request_id: 请求ID
    ...
    """
    logger.info(
        f"[{request_id}] Starting parallel batch processing: "
        f"{len(batches)} batches, max concurrent={max_concurrent}"
    )
    
    # 使用信号量控制并发
    semaphore = asyncio.Semaphore(max_concurrent)
    
    async def process_with_semaphore(batch_id: int, start_page: int, end_page: int):
    ...
    # 创建所有任务
    tasks = [
        process_with_semaphore(batch_id, start_page, end_page)
        for batch_id, (start_page, end_page) in enumerate(batches)
    ]
    ...
    for i, result in enumerate(batch_results):
        if isinstance(result, Exception):
            logger.error(f"[{request_id}] Batch {i} raised exception: {str(result)}")
            final_results.append({
                "batch_id": i,
                "start_page": batches[i][0],
                "end_page": batches[i][1],
                "result_dir": None,
                "success": False,
                "error": str(result)
            })
    ...
    if failed_count > 0:
        failed_batches = [r['batch_id'] for r in final_results if not r['success']]
        logger.warning(f"[{request_id}] Failed batches: {failed_batches}")
    
    return final_results


def merge_markdown_results(batch_results: list, file_stem: str, request_id: str = "unknown") -> str:
    """
    合并多个批次的markdown内容
    ...
    Returns:
        str: 合并后的markdown内容
    """
    logger.info(f"[{request_id}] Merging markdown from {len(batch_results)} batches")
    
    merged_content = []
    
    for result in sorted(batch_results, key=lambda x: x['batch_id']):
        if not result['success']:
            logger.warning(f"[{request_id}] Skipping failed batch {result['batch_id']} in markdown merge")
            merged_content.append(f"\n\n<!-- Batch {result['batch_id']} failed: {result['error']} -->\n\n")
            continue
        
        # 读取批次的markdown
        md_file = os.path.join(result['result_dir'], f"{file_stem}.md")
        if os.path.exists(md_file):
            ...
            logger.warning(f"[{request_id}] Markdown file not found for batch {result['batch_id']}: {md_file}")
    
    result = "\n".join(merged_content)
    logger.info(f"[{request_id}] Merged markdown: {len(result)} characters")
    return result


def merge_content_lists(batch_results: list, file_stem: str, request_id: str = "unknown") -> str:
    """
    合并多个批次的content_list
    ...
    Returns:
        str: 合并后的content_list JSON字符串
    """
    logger.info(f"[{request_id}] Merging content_list from {len(batch_results)} batches")
    
    merged_list = []
    
    for result in sorted(batch_results, key=lambda x: x['batch_id']):
        if not result['success']:
            logger.warning(f"[{request_id}] Skipping failed batch {result['batch_id']} in content_list merge")
            continue
        
        # 读取批次的content_list
        cl_file = os.path.join(result['result_dir'], f"{file_stem}_content_list.json")
        if os.path.exists(cl_file):
            try:
                ...
                if isinstance(content_list, list):
                    # 不需要调整page_idx，因为每个批次已经有正确的页码
                    merged_list.extend(content_list)
                else:
                    logger.warning(f"[{request_id}] Invalid content_list format in batch {result['batch_id']}")
                    
            except Exception as e:
                logger.error(f"[{request_id}] Failed to load content_list from batch {result['batch_id']}: {e}")
        else:
            logger.warning(f"[{request_id}] Content list file not found for batch {result['batch_id']}: {cl_file}")
    
    result = json.dumps(merged_list, ensure_ascii=False)
    logger.info(f"[{request_id}] Merged content_list: {len(merged_list)} items")
    return result


def merge_middle_jsons(batch_results: list, file_stem: str, request_id: str = "unknown") -> str:
    """
    合并多个批次的middle_json
    ...
    Returns:
        str: 合并后的middle_json字符串
    """
    logger.info(f"[{request_id}] Merging middle_json from {len(batch_results)} batches")
    ...
    for result in sorted(batch_results, key=lambda x: x['batch_id']):
        if not result['success']:
            logger.warning(f"[{request_id}] Skipping failed batch {result['batch_id']} in middle_json merge")
            continue
        
        # 读取批次的middle_json
        mj_file = os.path.join(result['result_dir'], f"{file_stem}_middle.json")
        if os.path.exists(mj_file):
            try:
                with open(mj_file, 'r', encoding='utf-8') as f:
                    middle_json = json.load(f)
                
                if isinstance(middle_json, dict) and "pdf_info" in middle_json:
                    # 合并pdf_info数组
                    merged_data["pdf_info"].extend(middle_json["pdf_info"])
                else:
                    logger.warning(f"[{request_id}] Invalid middle_json format in batch {result['batch_id']}")
                    
            except Exception as e:
                logger.error(f"[{request_id}] Failed to load middle_json from batch {result['batch_id']}: {e}")
        else:
            logger.warning(f"[{request_id}] Middle JSON file not found for batch {result['batch_id']}: {mj_file}")
    
    result = json.dumps(merged_data, ensure_ascii=False)
    logger.info(f"[{request_id}] Merged middle_json: {len(merged_data['pdf_info'])} pages")
    return result


async def merge_images(batch_results: list, file_stem: str, target_dir: str, request_id: str = "unknown") -> dict:
    """
    合并多个批次的图片
    ...
    Returns:
        dict: 图片文件名到路径的映射
    """
    logger.info(f"[{request_id}] Merging images from {len(batch_results)} batches")
    ...
    for result in sorted(batch_results, key=lambda x: x['batch_id']):
        if not result['success']:
            logger.warning(f"[{request_id}] Skipping failed batch {result['batch_id']} in image merge")
            continue
        
        # 查找批次的图片目录
        batch_image_dir = os.path.join(result['result_dir'], "images")
        if not os.path.exists(batch_image_dir):
            logger.debug(f"[{request_id}] No images directory in batch {result['batch_id']}")
            continue
        
        # 复制所有图片，重命名以避免冲突
        image_extensions = ['jpg', 'jpeg', 'png', 'bmp', 'gif', 'webp', 'tiff', 'tif', 'jp2']
        for ext in image_extensions:
            pattern = os.path.join(batch_image_dir, f"*.{ext}")
            for image_path in glob(pattern):
                try:
                    original_name = os.path.basename(image_path)
                    # 生成新的文件名以避免冲突
                    new_name = f"batch{result['batch_id']}_{original_name}"
                    target_path = os.path.join(target_dir, new_name)
                    
                    # 复制文件
                    shutil.copy2(image_path, target_path)
                    merged_images[original_name] = new_name
                    image_counter += 1
                    
                except Exception as e:
                    logger.error(f"[{request_id}] Failed to copy image {image_path}: {e}")
    
    logger.info(f"[{request_id}] Merged {image_counter} images from {len(batch_results)} batches")
...
async def merge_batch_results(
    ...
    Returns:
        dict: 合并后的结果信息
    """
    logger.info(f"[{request_id}] Starting merge of {len(batch_results)} batch results")
    ...
    # 合并markdown
    merged_md = merge_markdown_results(batch_results, file_stem, request_id)
    md_file = os.path.join(merged_dir, f"{file_stem}.md")
    with open(md_file, 'w', encoding='utf-8') as f:
        f.write(merged_md)
    
    # 合并content_list
    merged_cl = merge_content_lists(batch_results, file_stem, request_id)
    cl_file = os.path.join(merged_dir, f"{file_stem}_content_list.json")
    with open(cl_file, 'w', encoding='utf-8') as f:
        f.write(merged_cl)
    
    # 合并middle_json
    merged_mj = merge_middle_jsons(batch_results, file_stem, request_id)
    mj_file = os.path.join(merged_dir, f"{file_stem}_middle.json")
    with open(mj_file, 'w', encoding='utf-8') as f:
        f.write(merged_mj)
    
    # 合并图片
    merged_images_dir = os.path.join(merged_dir, "images")
    image_mapping = await merge_images(batch_results, file_stem, merged_images_dir, request_id)
    
    logger.info(f"[{request_id}] Batch results merged successfully to {merged_dir}")
    
    return {
        "result_dir": merged_dir,
        "markdown_file": md_file,
        "content_list_file": cl_file,
        "middle_json_file": mj_file,
...
async def file_parse(
    ...
    Now includes automatic batch processing for large PDFs (>50 pages) to prevent timeouts.
    """
    temp_dir = None
    ...
    try:
        ...
        try:
            ...
            # 为发票类文档优化VLM参数
            if actual_backend in VLM_BACKENDS_SET or actual_backend in HYBRID_BACKENDS_SET:
        ...
        # Set environment variables for VLM/Hybrid backend
        if actual_backend in HYBRID_BACKENDS_SET or actual_backend in VLM_BACKENDS_SET:
            if actual_backend in HYBRID_BACKENDS_SET:
                # For hybrid, formula_enable controls inline formula
                os.environ['MINERU_VLM_FORMULA_ENABLE'] = "true" # Always true for hybrid general formulas
                # We don't have a direct env var for inline formula in the common code, 
                # but it is passed as argument 'inline_formula_enable' in do_parse.
                # The 'formula_enable' argument in this API function will be passed 
                # as 'formula_enable' to do_parse, which maps it to 'inline_formula_enable' for hybrid.
                pass
            else:
                 os.environ['MINERU_VLM_FORMULA_ENABLE'] = str(formula_enable).lower()
            
            os.environ['MINERU_VLM_TABLE_ENABLE'] = str(table_enable).lower()
            logger.info(f"[{request_id}] Set VLM env vars: TABLE={table_enable}")

        # ============================================================================
        # Batch Processing Logic for Large PDFs
        # ============================================================================
        
        # Detect PDF page count for batch processing decision
        page_count = 0
        if enable_batch_processing:
            try:
                logger.info(f"[{request_id}] Detecting PDF page count for batch processing decision...")
                page_count = await get_pdf_page_count(pdf_bytes, request_id)
                logger.info(f"[{request_id}] Page count detection completed: {page_count} pages")
            except Exception as e:
                logger.warning(f"[{request_id}] Failed to detect page count: {e}, falling back to single batch")
        else:
            logger.info(f"[{request_id}] Batch processing is disabled via enable_batch_processing=False")
        
        # Decide whether to use batch processing
        should_use_batch = (
            enable_batch_processing and
            page_count > AUTO_BATCH_THRESHOLD and
            page_count > 0
        )
        
        logger.info(
            f"[{request_id}] Batch processing decision: "
            f"enabled={enable_batch_processing}, page_count={page_count}, "
            f"threshold={AUTO_BATCH_THRESHOLD}, will_use_batch={should_use_batch}"
        )
        
        if should_use_batch:
            logger.info(
                f"[{request_id}] Enabling batch processing for large PDF: "
                f"{page_count} pages > {AUTO_BATCH_THRESHOLD} threshold"
            )
            
            # 智能推算批处理参数（如果用户没有明确指定）
            # 检查用户是否提供了自定义参数（通过API）
            user_provided_batch_size = batch_size != BATCH_SIZE
            user_provided_max_concurrent = max_concurrent_batches != MAX_CONCURRENT_BATCHES
            
            if not user_provided_batch_size or not user_provided_max_concurrent:
                # 使用智能推算
                smart_params = calculate_smart_batch_params(page_count, request_id)
                
                # 如果用户没有提供batch_size，使用智能推算值
                if not user_provided_batch_size:
                    actual_batch_size = smart_params['batch_size']
                    logger.info(f"[{request_id}] Using smart batch_size: {actual_batch_size}")
                else:
                    actual_batch_size = batch_size
                    logger.info(f"[{request_id}] Using user-provided batch_size: {actual_batch_size}")
                
                # 如果用户没有提供max_concurrent，使用智能推算值
                if not user_provided_max_concurrent:
                    actual_max_concurrent = smart_params['max_concurrent']
                    logger.info(f"[{request_id}] Using smart max_concurrent: {actual_max_concurrent}")
                else:
                    actual_max_concurrent = max_concurrent_batches
                    logger.info(f"[{request_id}] Using user-provided max_concurrent: {actual_max_concurrent}")
            else:
                # 用户同时提供了两个参数，使用用户值
                actual_batch_size = batch_size
                actual_max_concurrent = max_concurrent_batches
                logger.info(
                    f"[{request_id}] Using user-provided parameters: "
                    f"batch_size={actual_batch_size}, max_concurrent={actual_max_concurrent}"
                )
            
            # Calculate batches
            batches = calculate_batches(page_count, actual_batch_size)
            logger.info(f"[{request_id}] Divided into {len(batches)} batches of ~{actual_batch_size} pages each")
            ...
            # Merge backend-specific params (but remove start/end page as they'll be set per batch)
            filtered_backend_params = {k: v for k, v in backend_params.items() 
                                      if k not in ['start_page_id', 'end_page_id']}
            batch_parse_params.update(filtered_backend_params)
            
            # Process batches in parallel
            batch_results = await process_batches_parallel(
                batches=batches,
                pdf_bytes=pdf_bytes,
            ...
            if not successful_batches:
                raise Exception("All batches failed during processing")
            
            # Merge batch results
            merge_result = await merge_batch_results(
            ...
            # Use merged result directory
            result_dir = merge_result['result_dir']
            logger.info(
                f"[{request_id}] Batch processing completed: "
                f"{merge_result['successful_batches']}/{merge_result['total_batches']} successful"
            )
            ...
            if page_count > 0:
                logger.info(
                    f"[{request_id}] Using single-batch processing: "
                    f"{page_count} pages <= {AUTO_BATCH_THRESHOLD} threshold"
                )
            else:
                logger.info(f"[{request_id}] Using single-batch processing (page count unknown)")
            
            # Parse document using original logic
...
async def get_mode_switch_info():
    ...
    return {
        ...
        "environment_variables": {
            ...
            "MINERU_BATCH_SIZE": {
                "description": "Pages per batch for large PDF processing",
                "default": 100,
                "current": BATCH_SIZE
            },
            "MINERU_MAX_CONCURRENT_BATCHES": {
                "description": "Maximum concurrent batches",
                "default": 2,
                "current": MAX_CONCURRENT_BATCHES
            },
            "MINERU_AUTO_BATCH_THRESHOLD": {
                "description": "Auto-enable batch processing threshold (pages)",
                "default": 50,
                "current": AUTO_BATCH_THRESHOLD
            }
        }
    }
...
if __name__ == "__main__":
    ...
    # Display batch processing configuration
    logger.info("=" * 60)
    logger.info("Batch Processing Configuration:")
    logger.info(f"  Auto-batch threshold: {AUTO_BATCH_THRESHOLD} pages")
    logger.info(f"  Batch size: {BATCH_SIZE} pages per batch")
    ...
    logger.info("  Pipeline mode: MINERU_MODE=pipeline python baba.py")
    logger.info("  VLM mode: MINERU_MODE=vlm MINERU_VLM_BACKEND=vlm-vllm-async-engine python baba.py")
    logger.info("=" * 60)
    
    # Batch processing configuration instructions
    logger.info("To configure batch processing:")
    logger.info(f"  MINERU_BATCH_SIZE=<pages> (default: {BATCH_SIZE})")
    logger.info(f"  MINERU_MAX_CONCURRENT_BATCHES=<num> (default: {MAX_CONCURRENT_BATCHES})")
    logger.info(f"  MINERU_AUTO_BATCH_THRESHOLD=<pages> (default: {AUTO_BATCH_THRESHOLD})")
    logger.info("Example:")
    logger.info("  MINERU_BATCH_SIZE=50 MINERU_MAX_CONCURRENT_BATCHES=3 python baba.py")
    logger.info("=" * 60)
