...
export const DatasetPageContextProvider = ({
  ...
  const updateDataset = async (data: UpdateDatasetBody) => {
    ...
    if (datasetId === data.id) {
      const { id: _, agentModel, vlmModel, apiDatasetServer, ...datasetUpdates } = data;

      setDatasetDetail((state) => ({
        ...state,
        ...datasetUpdates,
        agentModel: agentModel ? getWebLLMModel(agentModel) : state.agentModel,
        vlmModel: vlmModel ? getWebLLMModel(vlmModel) : state.vlmModel,
        apiDatasetServer: filterApiDatasetServerPublicData(apiDatasetServer)
      }));
    }
  };
