...
const Teams = ({ appId }: { appId: string }) => {
  const { t } = useTranslation();
  const { Loading, setIsLoading } = useLoading();
  const { feConfigs } = useSystemStore();
  const [editTeamsLinkData, setEditTeamsLinkData] = useState<OutLinkEditType<TeamsAppType>>();
  ...
  return (
    <Box position={'relative'} pt={3} px={5} minH={'50vh'}>
      <Flex justifyContent={'space-between'} flexDirection="row">
        ...
        <Button
          ...
          {...(shareChatList.length >= 10
            ...
            : {})}
          onClick={() => {
            setEditTeamsLinkData(defaultTeamsOutlinkForm);
            setIsEdit(false);
          }}
        >
          {t('common:add_new')}
            ...
            {shareChatList.map((item) => (
              <Tr key={item._id}>
                ...
                <Td display={'flex'} alignItems={'center'}>
                  <Button
                    onClick={() => {
                      setShowShareLink(`${baseUrl}/support/outLink/teams/${item.shareId}`);
      ...
      {editTeamsLinkData && (
        <TeamsEditModal
          appId={appId}
          defaultData={editTeamsLinkData}
          onCreate={() => Promise.all([refetchShareChatList(), setEditTeamsLinkData(undefined)])}
          onEdit={() => Promise.all([refetchShareChatList(), setEditTeamsLinkData(undefined)])}
          onClose={() => setEditTeamsLinkData(undefined)}
          isEdit={isEdit}
        />
      )}
      ...
      {showShareLinkModalOpen && (
        <ShowShareLinkModal
          ...
          onClose={closeShowShareLinkModal}
          img="/imgs/outlink/teams-copylink-instruction.png"
        />
      )}
    </Box>
  );
