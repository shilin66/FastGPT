---
title: '对话接口'
description: 'ZenMeta OpenAPI 对话接口'
icon: 'chat'
draft: false
toc: true
weight: 852
...
`v1`对话接口兼容`GPT`的接口！如果你的项目使用的是标准的`GPT`官方接口，可以直接通过修改`BaseUrl`和 `Authorization`来访问 ZenMeta 应用，不过需要注意下面几个规则：
...
- chatId: string | undefined 。
  - 为 `undefined` 时（不传入），不使用 ZenMeta 提供的上下文功能，完全通过传入的 messages 构建上下文。
  - 为`非空字符串`时，意味着使用 chatId 进行对话，自动从 ZenMeta 数据库取历史记录，并使用 messages 数组最后一个内容作为用户问题，其余 message 会被忽略。请自行确保 chatId 唯一，长度小于250，通常可以是自己系统的对话框ID。
- messages: 结构与 [GPT接口](https://platform.openai.com/docs/api-reference/chat/object) chat模式一致。
- responseChatItemId: string | undefined 。如果传入，则会将该值作为本次对话的响应消息的 ID，ZenMeta 会自动将该 ID 存入数据库。请确保，在当前`chatId`下，`responseChatItemId`是唯一的。
...
```bash
curl --location --request POST 'https://ops.zenlayer.ai/api/v2/chat/completions' \
--header 'Authorization: opsnb-xxxxxx' \
--header 'Content-Type: application/json' \
--data-raw '{
    "chatId": "my_chatId",
    "stream": false,
...
```bash
curl --location --request POST 'https://ops.zenlayer.ai/api/v1/chat/completions' \
--header 'Authorization: Bearer opsnb-xxxx' \
--header 'Content-Type: application/json' \
--data-raw '{
    "stream": true,
    "detail": true,
...
```bash
curl --location --request POST 'https://ops.zenlayer.ai/api/core/chat/getHistories' \
...
```bash
curl --location --request POST 'https://ops.zenlayer.ai/api/core/chat/updateHistory' \
--header 'Authorization: Bearer {{apikey}}' \
--header 'Content-Type: application/json' \
--data-raw '{
    "appId": "appId",
...
```bash
curl --location --request DELETE 'https://ops.zenlayer.ai/api/core/chat/delHistory?chatId={{chatId}}&appId={{appId}}' \
--header 'Authorization: Bearer {{apikey}}'
```
...
```bash
curl --location --request DELETE 'https://ops.zenlayer.ai/api/core/chat/clearHistories?appId={{appId}}' \
--header 'Authorization: Bearer {{apikey}}'
```
...
```bash
curl --location --request GET 'https://ops.zenlayer.ai/api/core/chat/init?appId={{appId}}&chatId={{chatId}}' \
--header 'Authorization: Bearer {{apikey}}'
```
{{< /markdownify >}}
{{< /tab >}}
...
```bash
curl --location --request POST 'https://ops.zenlayer.ai/api/core/chat/getPaginationRecords' \
--header 'Authorization: Bearer {{apikey}}' \
--header 'Content-Type: application/json' \
--data-raw '{
    "appId": "appId",
...
```bash
curl --location --request GET 'https://ops.zenlayer.ai/api/core/chat/getResData?appId={{appId}}&chatId={{chatId}}&dataId={{dataId}}' \
--header 'Authorization: Bearer {{apikey}}'
```
...
```bash
curl --location --request DELETE 'https://ops.zenlayer.ai/api/core/chat/item/delete?contentId={{contentId}}&chatId={{chatId}}&appId={{appId}}' \
--header 'Authorization: Bearer {{apikey}}'
```
...
```bash
curl --location --request POST 'https://ops.zenlayer.ai/api/core/chat/feedback/updateUserFeedback' \
--header 'Authorization: Bearer {{apikey}}' \
--header 'Content-Type: application/json' \
--data-raw '{
    "appId": "appId",
...
猜你想问，必须包含 appId 和 chatId 的参数才可以进行使用。会自动根据 chatId 去拉取最近 6 轮对话记录作为上下文来引导回答。
...
```bash
curl --location --request POST 'https://ops.zenlayer.ai/api/core/ai/agent/v2/createQuestionGuide' \
--header 'Authorization: Bearer {{apikey}}' \
--header 'Content-Type: application/json' \
--data-raw '{
    "appId": "appId",
