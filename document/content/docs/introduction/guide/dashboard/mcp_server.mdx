---
title: MCP 服务
description: 快速了解 OmniCockpit MCP server
---
...
OmniCockpit MCP Server 功能允许你选择`多个`在 OmniCockpit 上构建好的应用，以 MCP 协议对外提供调用 OmniCockpit 应用的能力。

目前 OmniCockpit 提供的 MCP server 为 SSE 通信协议，未来将会替换成 `HTTP streamable`。

## OmniCockpit 使用 MCP server

### 1. 创建 MCP server

登录 OmniCockpit 后，打开`工作台`，点击`MCP server`，即可进入管理页面，这里可以看到你创建的所有 MCP server，以及他们管理的应用数量。
...
可以在支持 MCP 协议的客户端使用这些地址，来调用 OmniCockpit 应用，例如：`Cursor`、`Cherry Studio`。