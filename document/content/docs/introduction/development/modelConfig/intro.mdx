---
title: OmniCockpit 模型配置说明
description: OmniCockpit 模型配置说明
---
...
OmniCockpit 页面上提供了每类模型的简单测试，可以初步检查模型是否正常工作，会实际按模板发送一个请求。
...
3. 打开 OmniCockpit 模型配置，新增一个`BAAI/bge-reranker-v2-m3`的重排模型（如果系统内置了，也可以直接变更,无需新增）。
